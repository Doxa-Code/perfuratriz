    ...
    <>
      ...
      >
        <span
          ...
          className="flex gap-3.5 align-center items-center leading-normal truncate"
        >
          {item.icon ? (
            <Icon
              icon={item.icon}
          ...
          {!isMenuItem ? (
            <div className="max-w-36 truncate hide-menu flex-1">
              {t(`${item.name}`)}
            </div>
          ) : (
            <div
              className={`max-w-36 truncate hide-menu flex-1 ${
                item.url == pathname ? "text-primary" : ""
              }`}
            >
              {t(`${item.name}`)}
